...
namespace GDF_HRMS_v1.HRMS_Mapper
{
    public class HRMSMappings : Profile
    {
        public HRMSMappings()
        {
           ...
           //     .ForMember(dest => dest.Address.Country.Id, opt => opt.MapFrom(src => src.CountryId))
           ...
           //     .ForPath(dest => dest.Address.Village, opt => opt.MapFrom(src => src.AddressVillage));
            ...
            CreateMap<CareerHistory, EmployeePIDto>()
                .ForPath(dest => dest.DepartmentName, opt => opt.MapFrom(src => src.Department.Name))
                 .ForPath(dest => dest.DepartmentLocation, opt => opt.MapFrom(src => src.Department.Location))
                 .ForPath(dest => dest.DepartmentDescription, opt => opt.MapFrom(src => src.Department.Description))
